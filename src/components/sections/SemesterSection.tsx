...
export default function SemesterSection() {
  ...
  const progressData = useMemo(() => {
    const totalECTS = coursesData.reduce(
    ...
    for (let semester = 1; semester <= 7; semester++) {
      ...
      const semesterTotalECTS = semesterCourses.reduce(
        (total, course) => total + course.ects,
        0,
      );
      ...
      semesterProgress[semester] = {
        ...
        percentage:
          semesterTotalECTS > 0
            ? (semesterCompletedECTS / semesterTotalECTS) * 100
            : 0,
      };
    }
      ...
      <div className="col-span-1 flex flex-col bg-[#121212] rounded-[48px] flex-nowrap gap-4 w-full justify-center overflow-hidden relative border border-[#292929]">
        <button
          ...
          className={`flex items-center justify-center w-full p-6 hover:bg-[#292929] transition-colors cursor-pointer relative ${
            expanded ? "border-b border-[#292929]" : ""
          }`}
        >
              ...
              <div className="w-full p-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-white">
                    {language === "da" ? "Færdiggjort:" : "Completed:"}{" "}
                    {progressData.completedECTS} ECTS
                  </span>
                  <span className="text-white">
                    Total: {progressData.totalECTS} ECTS
                  </span>
                </div>
                        ...
                        <div className="w-full mt-2">
                          <div className="flex justify-between items-center text-sm mb-1">
                            <span className="text-white">
                              {language === "da"
                                ...
                                : "Completed:"}{" "}
                              {
                                progressData.semesterProgress[semester]
                                  .completedECTS
                              }{" "}
                              ECTS
                            </span>
                          ...
                          <div className="w-full bg-[#292929] rounded-full h-3">
                            <div
                              ...
                              style={{
                                width: `${progressData.semesterProgress[semester].percentage}%`,
                              }}
                            ></div>
                          </div>
                          <div className="text-xs text-gray-500 mt-1">
                            {progressData.semesterProgress[
                              semester
                    ...
                    <AnimatePresence>
                      {isSemesterExpanded && (
                        ...
                        >
                          ...
                          {semesterCourses.map(
                            (course, courseIndex, courseArray) => {
                              const isLastCourseInSemester =
